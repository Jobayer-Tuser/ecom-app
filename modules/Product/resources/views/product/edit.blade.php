                            ...
                            <form method="post" action="{{ route('product.update', $product->id) }}" enctype="multipart/form-data">
                                ...
                                <div class="row mb-n3">
                                    <x-form class="col-xl-6" :title="'Product Name'">
                                        <x-input-text name="name" type="text" placeholder="Ex. Macbook Air M1" value="{{ old('name', $product->name) }}" />
                                    </x-form>
                                    ...
                                    <x-form class="col-xl-6" :title="'Select Category'">
                                        <x-input-select name="category_id" :type="'Category'" >
                                            @foreach($categories as $eachCategory)
                                                <option {{ old('category_id', $product->category_id) == $eachCategory->id ? 'selected' : '' }} value="{{ $eachCategory->id }}">{{ $eachCategory->name }}</option>
                                            @endforeach
                                        </x-input-select>
                                    </x-form>

                                    <x-form class="col-xl-12"  :title="'Product Summary'">
                                        <x-input-textarea name="summary" placeholder="Ex. Product Summary" >{{ old('summary', $product->summary) }}</x-input-textarea>
                                    </x-form>

                                    <x-form class="col-xl-12"  :title="'Product Description'">
                                        <x-input-textarea name="description" placeholder="Ex. Product Description" >{{ old('description', $product->description) }}</x-input-textarea>
                                    </x-form>

                                    <x-form class="col-xl-12"  :title="'Product Image'">
                                        <x-input-text name="image" type="file" placeholder="Ex. Macbook Air M1" value="{{ old('image', $product->image) }}" />
                                    </x-form>
                                </div>
                                <div class="row mt-1">
